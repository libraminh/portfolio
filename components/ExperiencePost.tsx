...
const ExperienceDate = ({ children }: { children: ReactNode }) => {
  return (
    <div className="inline-flex items-center justify-center rounded-full border boder-solid border-gray-500 dark:border-0 px-5 py-2 dark:bg-gray-400 bg-white dark:text-white text-gray-900 text-sm text-center">
      {children}
    </div>
  );
};
...
const ExperiencePost = ({ position, company, content, date }: Props) => {
  ...
  return (
    <article className="flex flex-col md:flex-row text-black dark:text-white space-y-3 md:space-y-0">
      ...
      <div className="hidden w-[1px] md:flex mx-4 dark:bg-white bg-gray-500"></div>
